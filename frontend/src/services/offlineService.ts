...
class OfflineService {
  private dbName = 'kyoboInventoryDB';
  private storeName = 'offlineChanges';
  ...
  async saveChange(change: { type: string; data: any }): Promise<void> {
  ...
  async queueInventoryAdjustment(productId: string, quantity: number, memo: string): Promise<void> {
  ...
  async queueLocationMove(productId: string, toLocation: string): Promise<void> {
    const moveData = {
      type: 'location_move',
  ...
  async safeProcessChanges<T>(processor: (changes: any[]) => Promise<T>): Promise<T | null> {
    try {
  ...
  async getChangesByType(type: string): Promise<any[]> {
    const changes = await this.getChanges();
    return changes
