...
export const aiService = {
  // 제품 설명 자동 생성
  async generateProductDescription(productName: string, category: string, brand: string): Promise<string> {
    if (!GEMINI_API_KEY) {
...
설명은 200자 이내로 작성하고, 제품의 특징과 장점을 포함해주세요.`;
      ...
      return geminiResponse.candidates[0]?.content?.parts[0]?.text || '설명을 생성할 수 없습니다.';
    } catch (error) {
  ...
  // 재고 최적화 제안
  async getInventoryOptimization(products: any[]): Promise<string> {
    if (!GEMINI_API_KEY) {
      throw new Error('Gemini API 키가 설정되지 않았습니다.');
    }
...
재고 관리 개선 방안과 구체적인 액션 아이템을 제안해주세요.`;
      ...
      const geminiResponse: GeminiResponse = response.data;
      return geminiResponse.candidates[0]?.content?.parts[0]?.text || '분석을 생성할 수 없습니다.';
    } catch (error) {
  ...
  // 판매 예측 및 분석
  async getSalesAnalysis(salesData: any[]): Promise<string> {
    if (!GEMINI_API_KEY) {
      throw new Error('Gemini API 키가 설정되지 않았습니다.');
    }
...
가장 적합한 카테고리 하나만 답변해주세요.`;
      ...
      const geminiResponse: GeminiResponse = response.data;
      return geminiResponse.candidates[0]?.content?.parts[0]?.text?.trim() || '기타';
    } catch (error) {
