...
exports.sendLowStockAlert = void 0;
const nodemailer_1 = __importDefault(require("nodemailer"));
const dotenv_1 = __importDefault(require("dotenv"));
const index_1 = require("./index"); // prisma 임포트 추가
 ...
 */
const sendLowStockAlert = async (productName, sku, currentStock, safetyStock) => {
    try {
        // 관리자 이메일 가져오기
        ...
        if (!admin) {
            logger_1.default.error('관리자 계정을 찾을 수 없습니다.');
            return;
        }
      ...
      `
        ...
        logger_1.default.info(`저재고 알림 이메일이 ${adminEmail}로 전송되었습니다.`);
    }
    catch (error) {
        logger_1.default.error('이메일 전송 중 오류 발생:', error);
    }
};
